...
class InvitationsPage extends BasePage {
  ...
  get inviteEmplooyeeBtn() {
    return cy.get("div a.export");
  }
  get emailID() {
    return cy.get("input[name=email]");
  }
  get employeeName() {
    return cy.get("input[name=employeeName]");
  }
  get chooseFile() {
  ...
  get submitBtn() {
    return cy.get(
      "div[class='justify-content-center modal-footer'] button[type='submit']"
    );
  }
  get onBoardingSuccessMsg() {
    return cy.get("div.Toastify__toast-body");
  }
  get onBoardingForm() {
  ...
  enterEmailID(randomEmail) {
    this.emailID.wait(500).type(randomEmail).should("have.value", randomEmail);
