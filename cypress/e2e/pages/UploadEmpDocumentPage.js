...
class UploadEmpDocumentPage extends BasePage {
  ...
  insuranceCardActBtn(text) {
    return cy.get(
      `tbody tr:has(td:nth-child(2):contains("${text}")) a i.fa-upload`
    );
  }
  get uploadeActPopLbl() {
  ...
  get cancelBtn() {
    return cy.get("div.modal-footer button[type='button'].cancel-promote");
  }
  get crossIcon() {
    return cy.get("div.modal-header button[type='button'].close-class");
  }
  get chooseFile() {
  ...
  eyeIconAnchor(value) {
    return cy.get(
      `tbody tr:has(td:nth-child(2):contains('${value}')) span.text-left a.fs-5`
    );
  }
