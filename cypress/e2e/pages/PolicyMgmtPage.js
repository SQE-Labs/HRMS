...
class PolicyMgmtPage extends BasePage {
  ...
  get editPolicyBtn() {
    return cy.get("button[data-bs-target='#staticBackdropPolicy']");
  }
  get addPolicyHeader() {
    return cy.get("#staticBackdropLabel");
  }
  get policyTitleTxt() {
    return cy.get("input[name='policyTitle']");
  }
  get policyDescTxt() {
  ...
  get cancelBtn() {
    return cy.get("button[type='button'].theme-button");
  }
  get itemPerPageDrp() {
  ...
  get lastPolicyDesc() {
    return cy.get("tbody tr:last-of-type td:nth-child(4)");
  }
  get lastValidDate() {
