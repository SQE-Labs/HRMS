...
class AssetAllocationPage extends BasePage {
  ...
  assetSelectedDetails() {
    cy.get('tbody')
      ...
      .first()
      .within(() => {
        cy.get('td:nth-child(1)').invoke('text').then((text1) => {
          const expectedId = text1.trim();

          cy.get('td:nth-child(2)').invoke('text').then((text2) => {
            const expectedManufacturer = text2.trim();

            cy.get('td:nth-child(3)').invoke('text').then((text3) => {
              const expectedAssetName = text3.trim();

              cy.get('td:nth-child(4)').invoke('text').then((text4) => {
                ...
                cy.get('td:nth-child(5)').invoke('text').then((text5) => {
                  ...
                  cy.xpath("//label[contains(text(),'Selected Asset')]//parent::div//textarea")
                    ...
                    .then((textareaValue) => {
                      ...
                      const textareaDetails = textareaValue.split('\n');

                      const idFromTextarea = textareaDetails[0].split(':')[1].trim();
                      const manufacturerFromTextarea = textareaDetails[1].split(':')[1].trim();
                      const serialNumberFromTextarea = textareaDetails[2].split(':')[1].trim();
                      const ownerFromTextarea = textareaDetails[3].split(':')[1].trim();
                      const assetNameFromTextarea = textareaDetails[6].split(':')[1].trim();
                     
                      // Step 4: Assertions to compare grid values with textarea values
                      expect(idFromTextarea).to.equal(expectedId);
                      expect(manufacturerFromTextarea).to.equal(expectedManufacturer);
                      expect(serialNumberFromTextarea).to.equal(expectedSerialNumber);
                      expect(ownerFromTextarea).to.equal(expectedOwner);
                      expect(assetNameFromTextarea).to.equal(expectedAssetName);
                    });
                });
              });
            });
