...
class EmployeeListPage extends BasePage {
  ...
  get user() {
    return cy.get(".card-title.text-primary");
  }
  get employeeCount() {
    return cy.get(".total");
  }
  get noRecordAvailable() {
    return cy.get(".fs-4.text-secondary.text-center");
  }
  get department() {
    return cy.get("#department");
  }
  get totalCount() {
    return cy.get("div[class='total'] span");
  }

  //Methods
  enterNameIntoSearchField(nametxt) {
    cy.get('#staticBackdrop').should('not.have.class', 'show');
  ...
  getTotalEmployeescount() {
    return this.employeeCount.invoke("text").then((text) => {
